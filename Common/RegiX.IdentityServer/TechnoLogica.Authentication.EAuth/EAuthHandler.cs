...
namespace TechnoLogica.Authentication.EAuth
{
    public class EAuthHandler : RemoteAuthenticationHandler<EAuthOptions>
    {
        ...
        protected async override Task<HandleRequestResult> HandleRemoteAuthenticateAsync()
        {
            ...
            {
                //viewModel.Messages.Add("Неуспешна идентификация. Възникна проблем с разчитането на Вашия КЕП.");
                //viewModel.Messages.Add("Заявката не идва от очакваното място.");

                //viewModel.HiddenMessages.Add(string.Format("Адрес: \"{0}\"", urlReferer));

                //await this.LogMessage(viewModel);
        ...
        public async Task<string> CreateSAMLRequest(HttpContext context)
        {
            await this.InitializeAsync(
               new AuthenticationScheme(
                   EAuthDefaults.AuthenticationScheme,
                   EAuthDefaults.DisplayName,
                   this.GetType()
                ),
                context
            );

            var request = Utils.GenerateNewSamlRequest(Options);

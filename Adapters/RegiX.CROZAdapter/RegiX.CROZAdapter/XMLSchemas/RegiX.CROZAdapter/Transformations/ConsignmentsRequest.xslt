		...
		<html>
			...
			<body>
				<xsl:for-each select="$XML">
					<xsl:for-each select="n1:ConsignmentsRequest">
						<h3 style="font-size:18px; text-align:center; ">
							<span>
								<xsl:text>Заявка за справка за вписванията по партида в Централния регистър на особените залози</xsl:text>
							</span>
						</h3>
						<br/>
						<xsl:for-each select="n1:ParticipantID">
							<span style="font-size:16px; ">
								<xsl:text>Служебен идентификатор на участник: </xsl:text>
							</span>

...
namespace TechnoLogica.RegiX.IaosMROBatteriesAdapter
{
    public partial class MRO_BA_Category_Response
    {
        public bool ShouldSerializeAuthorization()
        {
            bool haveSomeValue = this.Authorization != null &&
                ...
                this.Authorization.ShouldSerializeBACategories()
                );
    ...
    public partial class AuthorizationCategory
    {
        ...
        public bool ShouldSerializeBACategories()
        {
            bool haveSomeValue = this.BACategories != null &&
                (
                this.BACategories.ShouldSerializeBACategory()
                );
    ...
    {
        public bool ShouldSerializeBACategory()
        {
            //Here removing the empty elements from the array or collection
            if (this.BACategory != null)
            {
                this.BACategory = this.BACategory.Where(x => !string.IsNullOrEmpty(x)).ToList();
            }

            bool haveSomeValue = this.BACategory != null &&
                this.BACategory.Count() != 0;

            if (!haveSomeValue)
            {
                this.BACategory = null;
            }

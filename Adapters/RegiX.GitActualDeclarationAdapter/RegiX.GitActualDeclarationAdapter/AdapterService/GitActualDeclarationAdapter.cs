...
namespace TechnoLogica.RegiX.GitActualDeclarationAdapter.AdapterService
{
    ...
    [ExportFullName(typeof(GitActualDeclarationAdapter), typeof(IAdapterService))]
    [ExportSimpleName(typeof(GitActualDeclarationAdapter), typeof(IAdapterService))]
    public class GitActualDeclarationAdapter : SQLServerAdapterService.SQLServerAdapterService, IGitActualDeclarationAdapter, IAdapterService
    {
        ...
        [ExportFullName(typeof(GitActualDeclarationAdapter), typeof(ParameterInfo))]
        ...
        public CommonSignedResponse<AuthenticityExplosivesRequestType, RZZBUTDeclaration> GetActualDeclaration(AuthenticityExplosivesRequestType argument, AccessMatrix accessMatrix, AdapterAdditionalParameters aditionalParameters)
        {
            ...
            try
            {
                ...
                command.CommandText = @"exec " + MViewsUser.Value + ".[usp_Deklaracia15_sel] @eik ";

                command.Parameters.AddWithValue("@eik", argument.DeclaratorIdentifier);

                SqlDataAdapter resultDataSet = new SqlDataAdapter(command);
                ds.Tables.Add(ProcedureName.Value);

                ds.Tables.Add("EnterpriseUnits");
                ds.Tables.Add("EnterprisePersons");
                ds.Tables.Add("EnterpriseActivities");
                ds.Tables.Add("RegulatedRests");
                ds.Tables.Add("DangerousCancerAgents");
                ds.Tables.Add("DangerousChemicalAgents");

                try
                {
                    connection.Open();
                    resultDataSet.Fill(ds.Tables[ProcedureName.Value]);
                    FillHelpTables(ds);
                }
                finally
                {
                ...
                DataSetMapper<RZZBUTDeclaration> mapper = CreateMap(accessMatrix, argument.DeclaratorIdentifier);
        ...
        private void FillHelpTables(DataSet ds)
        {
            ...
            if (row != null)
            {
                this.FillDevisions(ds, row["podrazdelenia"].ToString(), "EnterpriseUnits");
                this.FillPersons(ds, row["upravliavashti"].ToString(), "EnterprisePersons");
        ...
        private void FillRests(DataSet ds, string xml, string tableName)
        {
            ...
            foreach (var item in r.DocumentElement.ChildNodes)
            {
                ...
                double procent = node.GetElementsByTagName("procent")[0] == null ? 0 :
                    double.Parse(node.GetElementsByTagName("procent")[0].InnerText, CultureInfo.InvariantCulture);
        ...
        private void FillPersons(DataSet ds, string xml, string tableName)
        {
            var doc = new XmlDocument();
            doc.InnerXml = string.Format("<root>{0}</root>", xml);
            ...
            currentTable.Columns.Add("TIP_DLAJNOST", typeof(bool));
            currentTable.Columns.Add("IS_PODAVASHT", typeof(bool));
            ...
            foreach (var item in doc.DocumentElement.ChildNodes)
            {
                ...
                bool tip = node.GetElementsByTagName("TIP_DLAJNOST")[0] == null ? false :
                    (node.GetElementsByTagName("TIP_DLAJNOST")[0].InnerText == "1");
                bool podavasht = node.GetElementsByTagName("IS_PODAVASHT")[0] == null ? false :
                    (node.GetElementsByTagName("IS_PODAVASHT")[0].InnerText == "1");
                ...
                row["TELEFON"] = phone;
                row["EMAIL"] = email;
                row["TIP_DLAJNOST"] = tip;
                row["IS_PODAVASHT"] = podavasht;
        ...
        private void FillDevisions(DataSet ds, string devisionsXml, string tableName)
        {
            var doc = new XmlDocument();
            ...
            currentTable.Columns.Add("R_IME", typeof(string));
            currentTable.Columns.Add("R_TELEFON", typeof(string));
            currentTable.Columns.Add("R_EMAIL", typeof(string));
            ...
            foreach (var item in doc.DocumentElement.ChildNodes)
            {
                ...
                string rName = node.GetElementsByTagName("R_IME")[0] == null ? null :
                    node.GetElementsByTagName("R_IME")[0].InnerText;
                string rTelefon = node.GetElementsByTagName("R_TELEFON")[0] == null ? null :
                    node.GetElementsByTagName("R_TELEFON")[0].InnerText;
                string rEmail = node.GetElementsByTagName("R_EMAIL")[0] == null ? null :
                    node.GetElementsByTagName("R_EMAIL")[0].InnerText;
        ...
        {
            // Възмозно е  [TIP_DLAJNOST] и [IS_PODAVASHT] да не са bool, ние ги получаваме от 2рия селект(имаме xml string)
            // IV_3_OCENEN_RISK, IV_4_VIDEO_DISPLEI, IV_7_POCHIVKA, IV_6_VIBRACII, IV_5_SHUM би трябвало да са bool(сега са инт)
            // в коментари има "?" за колоните които не съм сигурен
            // колоната ADRES я има в процедурата(select EnterpriseUnits), но не се ползва от мапинга
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.AsbestosData.BorderlineMeasureTaken, "P2_A_PREDPR_MERKI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.AsbestosData.EmployeeRegisterAvailable, "P2_A_REG_RABOTESHTI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.AsbestosData.HealthStatusEstimated, "P2_A_OCENKA_ZS", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.AsbestosData.LabourInspectionNotified, "P2_A_OVED_IT_RIKOZ", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.AsbestosData.LicenseIssued, "P2_A_RAZRESH_DEMONTAJ", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.AsbestosData.RiskEstimated, "P2_A_RISK_EKSPOZICIA", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.AsbestosData.TrainingProvided, "P2_A_OBUCHENIE", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.AsbestosUsed, "P2_AZBEST", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));

            //ПРИЛОЖЕНИЕ 2, ЧАСТ ІІ. КАНЦЕРОГЕНИ И МУТАГЕНИ
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsData.BorderlineCorresponding, "P2_KM_GRAN_STOINOSTI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsData.ClosedProductionSet, "P2_KM_ZATV_SISTEMA", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsData.ExposureDecreased, "P2_KM_NAM_EKSPOZICIA", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsData.ExposureEsitamted, "P2_KM_OCENKA_RISK_EKSP", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsData.GeneralSecuritySet, "P2_KM_KOL_SR_ZASHTITA", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsData.OtherMeasure, "P2_KM_DRUGI_TEXT");
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsData.OtherMeasureSet, "P2_KM_DRUGI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsData.PersonalPreventionSet, "P2_KM_LPS", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsData.PreventionMeasureNotTaken, "P2_KM_NIAMA_PREDPR_MERKI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsData.ReplacementSupplied, "P2_KM_MERKI_NAM_RISK", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsData.TrainingProvided, "P2_KM_OSIG_OBUCH", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsData.UpdatedEmployeeList, "P2_KM_OPASTNOST_EKSPL", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsData.WarningSignsProvided, "P2_KM_OSIG_ETIKIRANE", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.CancerAgentsUsed, "P2_KACEROGENI_MUTAGENI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            //ПРИЛОЖЕНИЕ 2, ЧАСТ І. ХИМИЧНИ АГЕНТИ
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.ActionPlanPrepared, "P2_PLAN_PREDOTV_AVARII", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.ChemicalAgentsQuantityRestricted, "P2_OGR_HIM_AG", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.EmployeeCountResticted, "P2_OGR_BROI_RAB", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.ExposureDurationDecreased, "P2_NAMAL_PRODALJ", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.GeneralSecuritySet, "P2_MERKI_KOL_ZASHT", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.HealthcareProvided, "P2_ZDRAVNO_NABL", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.OtherMeasure, "P2_DRUGI_TEXT");
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.OtherMeasureSet, "P2_DRUGI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.PeriodicTestDone, "P2_PERIOD_IZMERV", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.PersonalPreventionSet, "P2_LPS", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.PreventionMeasureNotTaken, "P2_NE_SA_PREDPR_MERKI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.ProcedureManagementSet, "P2_SAHR_TRANSP", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.ReplacementSupplied, "P2_IZP_BEZOP_RPOC", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.RiskEstimated, "P2_OCENKA_RISK", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.SafetyIndexBox, "P2_KARTOTEKA_ILB", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsData.TrainingProvided, "P2_OSIG_OBUCHENIE", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.Appl2_CheminalCancerAsbestosAgentsData.ChemicalAgentsUsed, "P2_HIM_AGENTI");

            //Приложение 2: 4. Идентифицирани опасни химични агенти
            ...
            //3.
            mapper.AddDataRowMap((r) => r.RepresentativeDetails, (dr) => dr.Table.DataSet.Tables["EnterprisePersons"].Rows);
            mapper.AddDataColumnMap((r) => r.RepresentativeDetails[0].Names, "IMENA");
            mapper.AddDataColumnMap((r) => r.RepresentativeDetails[0].Position, "DLAJNOST");
            mapper.AddDataColumnMap((r) => r.RepresentativeDetails[0].Phone, "TELEFON");
            mapper.AddDataColumnMap((r) => r.RepresentativeDetails[0].Email, "EMAIL");
            mapper.AddDataColumnMap((r) => r.RepresentativeDetails[0].IsManager, "TIP_DLAJNOST"); // ?
            mapper.AddDataColumnMap((r) => r.RepresentativeDetails[0].IsRepresentative, "IS_PODAVASHT"); // ?

            //16. Документирана оценка на рисковете за безопасността и здравето на работещите
            mapper.AddDataColumnMap((r) => r.DocumentedEstimation, "III_16", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.EmployerHiring, "I_SAMOSTO_NAEMAT_RAB", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.EmployerIdentifier, "I_EIK");
            mapper.AddDataColumnMap((r) => r.EmployerPersonFullName, "I_IMENA_FL");
            mapper.AddDataColumnMap((r) => r.EmployerTemporaryWork, "I_ZAEMANI_RABOTNICI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            //8. Поделения на предприятието извън адреса по т. 50
            mapper.AddDataColumnMap((r) => r.EnterpriseUnitAvailable, "II_PERSONAL_BROI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.EnterpriseUnit[0].EnterpriseUnitManager.Email, "R_EMAIL");
            mapper.AddDataColumnMap((r) => r.EnterpriseUnit[0].EnterpriseUnitManager.FirstMiddleLastName, "R_IME");
            mapper.AddDataColumnMap((r) => r.EnterpriseUnit[0].EnterpriseUnitManager.Phone, "R_TELEFON");
            //mapper.AddDataColumnMap((r) => r.EnterpriseUnit[0].EnterpriseUnitManager.Position, "");
            ...
            mapper.AddDataColumnMap((r) => r.LabourCommitteAndGroup, "III_14_3", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));

            //14. Учреден комитет по условия на труд
            mapper.AddDataColumnMap((r) => r.LabourCommittee, "III_14_1", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));

            //14. група по условия на труд
            mapper.AddDataColumnMap((r) => r.LabourGroup, "III_14_2", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));

            //20. Провеждане на задължителни предварителни и периодични медицински прегледи
            mapper.AddDataColumnMap((r) => r.LabourMedicalVisit, "III_20", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.LabourConditions.ContsructionRiskAvailable, "IV_1_SKELETA", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.LabourConditions.ElectricityRiskAvailable, "IV_1_ELTOK", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.LabourConditions.HighVoltageRiskAvailable, "IV_1_EL_UREDBI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.LabourConditions.LiftingEquipmentRiskAvailable, "IV_1_POV_SAORAJENIA", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.LabourConditions.MachineEquipmentRiskAvailable, "IV_1_MASHINI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.LabourConditions.MovingPlatformRiskAvailable, "IV_1_PLATFORMI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.LabourConditions.OtherRiskAvailable, "IV_1_DRUGI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.LabourConditions.OtherRiskDescription, "IV_1_DRUGI_TEXT");
            ...
            mapper.AddDataColumnMap((r) => r.LabourConditions.PressureRiskAvailable, "IV_1_S_NALIAGANE", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.LabourConditions.RopeWayRiskAvailable, "IV_1_V_LINII", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.LabourConditions.ShiftWorkCount, "IV_1_RABOTA_SMENI_BROI");
            mapper.AddDataColumnMap((r) => r.LabourConditions.TransportMeansRiskAvailable, "IV_1_TR_SREDSTVA", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.LabourMedicineService, "III_15", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.ManualWeightWork.MeasuresNotTaken, "IV_3_NE_SA_PREDPRIETI_MERKI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.ManualWeightWork.MedicalEvaluationCount, "IV_3_MED_OCENKA_BROI");
            mapper.AddDataColumnMap((r) => r.ManualWeightWork.OrganisationalMeasuresAdditionalInformation, "IV_3_INFO", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.ManualWeightWork.OrganisationalMeasuresConsultations, "IV_3_KONSULTACII", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.ManualWeightWork.OrganisationalMeasuresOthers, "IV_3_DRUGI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.ManualWeightWork.OrganisationalMeasuresOthersText, "IV_3_DRUGI_TEXT");
            mapper.AddDataColumnMap((r) => r.ManualWeightWork.OrganisationalMeasuresTrainings, "IV_3_OBUCHENIE", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.ManualWeightWork.OrganisationalMeasuresTaken, "IV_3_PREDPRIETI_MERKI_ORG", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.ManualWeightWork.TechnicalEquipment, "IV_3_TEHN_SREDSTVA");
            mapper.AddDataColumnMap((r) => r.ManualWeightWork.TechnicalEquipmentSupplied, "IV_3_PREDPRIETI_MERKI_TECH", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.NoiseConditions.AverageWeeklyNoiseExposureData, "IV_5_SEDMICHNO_NIVO", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.CombinedNoiseAndOtotoxicExposure, "IV_5_SHUM_I_OTTOTOKS_MAT", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.CombinedNoiseAndVibrationExposure, "IV_5_SHUM_I_VIBRACII", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.ConstantNoiseExposureRisk, "IV_5_POSTOIANEN_SHUM", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.DailyNoiseExposureData, "IV_5_DNEVNO_NIVO", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.NoiseConditions.ExceptionAvailable, "IV_5_IZKL_CHLEN10", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.ExceptionReason, "IV_5_IZKL_CHLEN10_OSNOVANIE");
            mapper.AddDataColumnMap((r) => r.NoiseConditions.NoiseExposureEscapeEquipment, "IV_5_PODH_OBORUDVANE", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.NoiseExposureEscapeLps, "IV_5_LPS", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.NoiseExposureEscapeMaintenance, "IV_5_PODDRAJKA_OBOR", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.NoiseExposureEscapeNewMethods, "IV_5_METOD_S_PO_NISAK_SHUM", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.NoiseExposureEscapeOrganization, "IV_5_ORGANIZ_RAB", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.NoiseExposureEscapeOthers, "IV_5_DRUGI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.NoiseExposureEscapeOthersText, "IV_5_DRUGI_TEKST");
            mapper.AddDataColumnMap((r) => r.NoiseConditions.NoiseExposureEscapeTechnicalEquipment, "IV_5_TEHN_SREDSTVA", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.NoiseExposureEscapeWorkPlaces, "IV_5_RAZPOLOJENIE_RM", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.NoiseExposureEscapeNoMeasures, "IV_5_NE_SA_REDPR_MERKI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.NoiseExposureRiskEstimated, "IV_5_SHUM");
            mapper.AddDataColumnMap((r) => r.NoiseConditions.OtherNoiseExposureData, "IV_5_DRUGO", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.NoiseConditions.OtherNoiseExposureDataText, "IV_5_DRUGO_TEXT");
            mapper.AddDataColumnMap((r) => r.NoiseConditions.VariableNoiseExposureRisk, "IV_5_PROMENLIV_SHUM", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            //45. ЧАСТ VІ. Вибрации 
            mapper.AddDataColumnMap((r) => r.VibrationConditions.ExceptionAvailable, "IV_6_IZKL_CHLEN9", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            ...
            mapper.AddDataColumnMap((r) => r.VibrationConditions.OtherMeasures, "IV_6_DRUGI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.VibrationConditions.OtherMeasuresDescription, "IV_6_DRUGI_TEKST");
            mapper.AddDataColumnMap((r) => r.VibrationConditions.VibrationExposureEscapeAdditionalEquipment, "IV_6_DOP_OBORUDVANE", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.VibrationConditions.VibrationExposureEscapeClimateConditions, "IV_6_MIKROKL_USL", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.VibrationConditions.VibrationExposureEscapeLimits, "IV_6_OGRAN_PRODALJ", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.VibrationConditions.VibrationExposureEscapeLps, "IV_6_LPS", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.VibrationConditions.VibrationExposureEscapeMaintenance, "IV_6_PODD_OBORUDVANE", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.VibrationConditions.VibrationExposureEscapeMethods, "IV_6_METODI_RABOTA", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.VibrationConditions.VibrationExposureEscapeNoMeasures, "IV_6_NE_SA_PREDPR_MERKI", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));
            mapper.AddDataColumnMap((r) => r.VibrationConditions.VibrationExposureEscapeWorkEquipment, "IV_6_OBORUDVANE", (o) => ((int.TryParse(o.ToString(), out valueForParsing) && !o.Equals(0))));

...
{
    public partial class REG_UM_PATENT_PatentDetails_Response
    {
        public bool ShouldSerializePatentDetails()
        {
            //Here removing the empty elements from the array or collection
            if (this.PatentDetails != null)
            {
                this.PatentDetails = this.PatentDetails.Where(x => x != default(PatentType)).ToList();
            }

            bool haveSomeValue = this.PatentDetails != null &&
                this.PatentDetails.Count() != 0 &&
                (
                this.PatentDetails[0].ApplicationDate != default(DateTime) ||
                this.PatentDetails[0].ApplicationDateSpecified != default(Boolean) ||
                this.PatentDetails[0].RegistrationDate != default(DateTime) ||
                this.PatentDetails[0].RegistrationDateSpecified != default(Boolean) ||
                this.PatentDetails[0].ExpiryDate != default(DateTime) ||
                this.PatentDetails[0].ExpiryDateSpecified != default(Boolean) ||
                this.PatentDetails[0].PatentCurrentStatusDate != default(DateTime) ||
                this.PatentDetails[0].PatentCurrentStatusDateSpecified != default(Boolean) ||
                this.PatentDetails[0].ShouldSerializeApplicationNumber() ||
                this.PatentDetails[0].ShouldSerializeRegistrationNumber() ||
                this.PatentDetails[0].ShouldSerializePatentTitle() ||
                this.PatentDetails[0].ShouldSerializePatentDescription() ||
                this.PatentDetails[0].ShouldSerializePatentRepresentationSheetDetails() ||
                this.PatentDetails[0].ShouldSerializeIPCClassDetails() ||
                this.PatentDetails[0].ShouldSerializePriorityDetails() ||
                this.PatentDetails[0].ShouldSerializeExhibitionPriorityDetails() ||
                this.PatentDetails[0].ShouldSerializePublicationDetails() ||
                this.PatentDetails[0].ShouldSerializeApplicantDetails() ||
                this.PatentDetails[0].ShouldSerializeRepresentativeDetails() ||
                this.PatentDetails[0].ShouldSerializeInventorDetails()
                );
